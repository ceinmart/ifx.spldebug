...
spldbg_logged() {
    ...
    local logfile="$SPLDBG_ROOT/bin/outputs/${mode}-v0.1.0-$(date -u +%Y%m%dT%H%M%SZ)-$$.log"
    local -a result
    set +e
    (
        set -e
        echo "spldbg_version=0.1.0 mode=$mode utc=$(date -u +%FT%TZ)"
        ...
        # Hashes identificam o fonte efetivamente testado, mesmo antes do commit.
